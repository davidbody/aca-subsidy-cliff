...
The Affordable Care Act (ACA) contains an insidious feature that can cost (or save) people thousands of dollars based on a potentially small change in their income. People purchasing health insurance through the ACA qualify for premium subsidies if their income is less than 400% of the federal poverty level (FPL). These subsidies can be substantial, even tens of thousands of dollars per year. Once household income rises above 400% of the FPL, even by a dollar, no subsidy is available, and any subsidies already received must be repaid in whole. This subsidy "cliff" amounts to a an effective marginal income tax rate in excess of 100% and can create a disincentive to work for those affected. We describe how subsidies and the subsidy "cliff" are calculated and look at how the size of the cliff varies geographically and with age.
...
It is worth emphasizing that subsidies are primarily driven by premiums. Since the available susidy is the excess of the reference premium over a percent of a household's income, the available subsidy rises dollar for dollar with the reference premium. High premiums therefore translate directly into high subsidies.
...
### The cliff is high when subsidies are high

Why are the cliff amounts so high? The cliff amount is simply the available subsidy at 400% of FPL. High premiums translate into high subsidies which translate into high cliff amounts.

### Variation in cliff amounts

We've already seen that cliff amounts vary by age because premiums (and therefore subsidies) vary by age.
...
```{r, echo=FALSE}
c30 <- calc_cliff("Couple", 30, 2)
c60 <- calc_cliff("Couple", 60, 0)
```

For a couple age 30 with 2 children, the 2018 cliff ranges from $`r format(round(min(c30$cliff)), big.mark=",")` to $`r format(round(max(c30$cliff)), big.mark=",")` depending on the county in which the couple lives. Here are summary statistics for the size of the cliff across all counties.

```{r, echo=FALSE}
summary(c30$cliff, digits = 5)
```

For a couple age 60 with no children, the 2018 cliff ranges from $`r format(round(min(c60$cliff)), big.mark=",")` to $`r format(round(max(c60$cliff)), big.mark=",")`, with the following summary statistics:

```{r, echo=FALSE}
summary(c60$cliff, digits = 5)
```
The following histograms show that distributions for both examples are skewed, with cliff amounts generally higher for the older household.

```{r, echo=FALSE}

ggplot(rbind(c30, c60), aes(x = cliff)) +
  geom_histogram(bins = 30) +
  facet_wrap(~ age, ncol = 1, labeller = as_labeller(c(`30` = "Couple age 30, 2 children", `60` = "Couple age 60, no children"))) +
  labs(title = "Histogram of 2018 ACA Subsidy \"Cliff\"") + xlab("ACA Subsidy Cliff") +
  ylab("Number of counties") +
  theme_minimal()
```

The following maps show how the size of the cliff varies geographically for both example households.
...
* Premium subsidies are available to people with household incomes between 100% and 400% of FPL.
* Once household income exceeds 400% of FPL, no subsidy is available and any subsidy received must be repaid.
* High premiums translate into high subsidies.
* High subsidies translate into a high "cliff" at 400% of FPL.
